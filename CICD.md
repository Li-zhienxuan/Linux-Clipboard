...
* CNB 是所有工作流配置文件都存储在仓库根目录名为 [.cnb.yml](../quick-start.md) 的文件中（也可以通过 [include](../grammar.md#include) 关键字在文件中导入其他配置文件）。
...
::: tip
注： `&`，`*` 是 `YAML` 的语法，用与配置复用，详见 [YAML高级语法](../simplify-configuration.md#yaml-gao-ji-yu-fa)
:::

## 构建触发规则

在 GitHub Actions 中，通过指定 `on` 字段定义工作流触发的规则。 在 CNB 中，您可以通过编写`触发分支`、`触发事件`来定义构建流水线被触发的规则。详见 [构建触发规则](../trigger-rule.md)。
...
* 在 CNB 中，Runner指的是一个构建节点（默认是一个运行docker容器的节点），目前CNB提供的官方托管的构建节点，仅支持Linux系统的docker容器作为Runner。而CNB的企业版（私有化版本）, 可以自行接入不同的机器(`macOS`、`Windows`、`Linux`)。具体详见 [构建节点](/zh/build/build-node.md)。
...
main:
  push:
    - runner:
        tags: cnb:arch:amd64 # 指定在 amd64 架构构建节点上执行
...
* 在 CNB 中，工作流是运行在一个或多个可以指定 docker 镜像的 docker 容器中。在构建过程中，只需要在配置文件中指定所需镜像或 Dockerfile 文件，即可完成构建环境的安装。具体详见 [构建环境](../build-env.md)。
...
* 在 CNB 中，工作流默认就会拉取仓库代码，无需显示声明。详细配置方式见 [流水线语法-仓库配置](../grammar.md#git)。
...
* 在 CNB 中，工作流使用`if`,`ifModify`,`ifNewBranch`等字段设置步骤执行条件。详见 [流水线语法-if](../grammar.md#stage-if)。
...
* 在 CNB 中，工作流使用`cnb:await`,`cnb:resolve`两个内置任务，来定义工作流依赖关系和传递变量。详见 [await-resolve内置任务](../internal-steps/#cnb-await-resolve)
...
* 在 CNB 中，工作流使用`env`字段指定变量，用`imports`字段来引用外部仓库或密钥仓库文件作为环境变量。详见 [引用变量](../grammar.md#pipeline-imports) 和 [密钥仓库](../../repo/secret.md)。
...
* 在 CNB 中，工作流不支持这种策略。但可以使用YAML的锚点功能，模拟矩阵策略。参考 [YAML高级语法](../simplify-configuration.md#yaml-gao-ji-yu-fa)
...
* 在 CNB 中，工作流使用`pipeline.runner.volumes`声明`节点本地缓存`或者使用`docker:cache`内置任务来声明`远端docker镜像缓存`，来存放构建过程中的依赖缓存或中间产物。详见 [流水线缓存](../pipeline-cache.md)
...
* 在 CNB 中，工作流使用`cnbcool/attachments:latest`插件上传制品，详见 [附件插件](../../../plugin/#public/cnbcool/attachments)
...
* 在 CNB 中，工作流可以通过声明`docker`服务, 直接使用在任务里执行`docker`或`docker compose`命令，来启动数据库及服务容器。详见 [docker服务](../grammar.md#service-docker)
